...
export default function App() {
  ...
  return <main>
    ...
    {filters.view === 'milestones' ? <section className="cards" aria-label="Meilensteine">{milestones.map((item) => <article className="card" key={item.id}><div className="card-title"><span>{item.id}</span><span className={`health ${item.health}`}>{healthLabels[item.health]}</span></div><h2>{item.title}</h2><Progress value={item.progress} /><dl><div><dt>Geplanter Aufwand</dt><dd>{item.plannedEffortPt} PT</dd></div><div><dt>Berücksichtigt</dt><dd>{item.effortPt} PT</dd></div><div><dt>Arbeitspakete</dt><dd>{item.workPackageCount}</dd></div></dl></article>)}</section> : <section className="table-wrap"><table><thead><tr><th>ID</th><th>Arbeitspaket</th><th>Bereich</th><th>Meilenstein</th><th>Priorität</th><th>Status</th><th>Warnstufe</th><th>PT</th><th>Details</th></tr></thead><tbody>{workPackages.map((item) => <><tr key={item.id}><td>{item.id}</td><td>{item.title}</td><td>{item.area}</td><td>{item.milestoneId}</td><td>{report.priorityModel[item.priority]}</td><td>{item.progress}% · {item.status}</td><td><span className={`health ${item.health}`}>{healthLabels[item.health]}</span></td><td>{item.effortPt}</td><td>{item.featureSummary && <button className="link" onClick={() => setDetails(details === item.id ? null : item.id)} aria-expanded={details === item.id}>Details</button>}</td></tr>{details === item.id && <tr className="details"><td colSpan={9}><p>{item.featureSummary}</p>{item.dependsOn.length > 0 && <p><b>Abhängig von:</b> {item.dependsOn.join(', ')}</p>}<DetailList title="Akzeptanzkriterien" entries={item.acceptanceCriteria} /><DetailList title="To-dos" entries={item.todos} /></td></tr>}</>)}</tbody></table></section>}
