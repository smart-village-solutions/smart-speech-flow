import type { ProjectHealth, ProjectStatusReport, WorkPackage } from './project-status';
...
export type MilestoneRow = { id: string; title: string; plannedEffortPt: number; effortPt: number; progress: number; health: ProjectHealth; workPackageCount: number };
...
export const deriveMilestones = (report: ProjectStatusReport, items: WorkPackageRow[], filters: Filters): MilestoneRow[] => report.milestones
  .map((milestone) => {
    ...
    const effortPt = matching.reduce((total, item) => total + item.effortPt, 0);
    const progress = effortPt === 0 ? 0 : Math.round(matching.reduce((total, item) => total + item.effortPt * item.progress, 0) / effortPt);
    ...
    return { id: milestone.id, title: milestone.title, plannedEffortPt: milestone.plannedEffortPt, effortPt, progress, health, workPackageCount: items.filter((item) => item.milestoneId === milestone.id).length };
  })
