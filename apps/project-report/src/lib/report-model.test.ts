...
const report = {
  ...
  milestones: [{ id: 'M1', title: 'Test', plannedEffortPt: 3, sortOrder: 1, workPackages: [
    { id: 'WP-1', title: 'Done', area: 'Test', priority: 'must', effortPt: 2, status: 'done', health: 'needs_attention', dependsOn: [] },
    { id: 'WP-2', title: 'Idea', area: 'Test', priority: 'valuable', effortPt: 1, status: 'idea', health: 'blocked', dependsOn: [] }
  ] }]
...
describe('deriveMilestones', () => it('excludes ideas from effort, progress, and health', () => {
  ...
  expect(result).toMatchObject({ effortPt: 2, progress: 100, health: 'needs_attention', workPackageCount: 2 });
}));
