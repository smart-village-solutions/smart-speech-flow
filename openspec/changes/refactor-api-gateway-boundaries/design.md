...
| Conversation service (`ConversationService`) | `build_gateway_dependencies`, with this app's session manager, speech pipeline, pipeline admission, quality telemetry and WebSocket manager | `get_conversation_service` | container |
...
| Speech pipeline (`SpeechPipeline`: `HttpSpeechServices` over this app's breakers, and the translation refiner) | `build_gateway_dependencies` | `get_speech_pipeline`, for `POST /pipeline` and `POST /upload`; the conversation service by constructor | container |
