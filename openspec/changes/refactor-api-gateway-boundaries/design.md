...
Existing app-state services migrate into the container without behavioral changes. Global objects remained temporary adapters until their consumers migrated; since PR7b none is left.

#### Dependency ownership

`create_app()` in `app.py` builds one app, with the collaborators that must outlive a lifespan: its Prometheus series and its rate limits. Its lifespan builds that app's `GatewayDependencies` (`services/api_gateway/dependencies.py`), keeps it at `app.state.dependencies`, and sets that to `None` on shutdown. `app = create_app()` stays the module attribute uvicorn and the Dockerfile target.
...
| Audio store (`AudioStore`) | `build_gateway_dependencies` (`AudioStore.from_environment()`, which reads `SSF_AUDIO_BASE_DIR` as it runs), counting into this app's `AudioStorageMetrics` | none; the conversation service and the session manager by constructor, `audio_cleanup_task` from the lifespan's container | container |
...
| WebSocket monitor (`WebSocketMonitor`) | `build_gateway_dependencies`, with this app's pseudonymizer, counting into this app's `WebSocketMetrics` | `get_connection_monitor`, for `/api/websocket/monitoring/health`; the WebSocket manager by constructor | container |
...
| Prometheus registry and metric objects (`GatewayMetrics`: the registry, the request counter, the pipeline admission, refinement and realtime series, the polling drop counter and `AudioStorageMetrics`) | `create_app()` (`GatewayMetrics.build()`), kept at `app.state.gateway_metrics`, its registry also at `app.state.prometheus_registry`; the lifespan hands the series to the components it builds, and the container receives the registry | `get_prometheus_registry` | app |
| OIDC key cache (`OidcKeyCache`) | `build_gateway_dependencies` | `get_oidc_key_cache` | container |
| Rate limits (`RateLimits`: the config and both sliding windows) | `create_app()`, kept at `app.state.rate_limits` | none; `RateLimitMiddleware` by keyword argument | app |
...
- A collaborator in the "app" rows is built once per app by `create_app()` and kept by every lifespan of that app; everything else is built per lifespan.
- What stays process-wide is the module attribute `app = create_app()`, which uvicorn and the Dockerfile start, and configuration read at import: the speech-service URLs and schemes (`DOCKER_COMPOSE`, `SERVICE_SCHEME`, `LOCAL_SERVICE_SCHEME`). No collaborator is process-wide.
- `tests/test_gateway_dependency_ownership.py` enforces the first two rules. Its allowlist holds one permanent entry, `app`, and no adapter (PR7b deleted `fallback_manager` with its module); an entry must now read `permanent: <reason>`. Its second list, of module globals rebound through a `global` statement, is empty, so a new one fails it. Both lists only shrink.
- Two apps in one process share no collaborator: `tests/test_gateway_app_isolation.py` holds them to that, their metrics and rate limits included. Each app owns its session manager, so termination revokes that app's tickets and notifies that app's pollers and sockets.

#### Last adapters (PR7a)

- `GatewayMetrics` (`gateway_metrics.py`) is one app's series. `create_app()` builds it: a fresh `CollectorRegistry`, `gateway_requests_total` incremented by zero so it is exposed from the first scrape, `PipelineAdmissionMetrics`, `RefinementMetrics`, `WebSocketMetrics` and the polling drop counter on that registry, and `AudioStorageMetrics` on a registry of its own. The lifespan hands the admission gate, the refiner, the WebSocket monitor, the polling store and the audio store their series from it. Nothing registers at import, so no module reload or second app raises "Duplicated timeseries".
- A second lifespan on one app reuses the app's `GatewayMetrics`: the series keep counting and nothing registers twice. The series a lifespan registers itself (`RuntimePolicyMetrics`, `FeedbackMaintenanceMetrics`, the quality telemetry counter) reuse what the app's registry already holds, as they did on the module registry. Two apps share no registry and no series, so one app's `/metrics` never shows another's counts.
- `AudioStorageMetrics` (`audio_storage.py`) holds the disk usage and file gauges and the cleanup counter, which `AudioStore.cleanup_expired` and `disk_usage` now record; the v2 functions no longer touch a metric. They used to live on prometheus_client's default registry, which `/metrics` never served, and they still count into a registry nothing serves. `monitoring/alert_rules.yml`'s four audio alerts have no data either way, and serving them changes what production alerts on (#426). `PROMETHEUS_AVAILABLE` is gone: `prometheus-client` is pinned in the gateway's requirements and imported unconditionally by `app.py` and eleven other gateway modules, so its `ImportError` branch could not run.
- The OIDC key cache is built by `build_gateway_dependencies`, so each lifespan starts with no keys and fetches an issuer's on its first bearer token, as a new process always has. Its five-minute lifetime per issuer and its refetch after that are unchanged.
- `RateLimits` (`rate_limiter.py`) is built once per app by `create_app()` and handed to `RateLimitMiddleware` as `limits=`. The state was per app before too, built once with the middleware stack; every lifespan of an app keeps it. Limits, windows, keys and 429 bodies are unchanged. `LATEST_RATE_LIMIT_MIDDLEWARE` is gone, as is the `rate_limit_middleware` attribute the middleware set on the ASGI app it wraps, which has no `state` and which nothing read.
- `tests/gateway_contract/test_contract_metrics_surface.py` pins every family `/metrics` exposes after a production-shaped flow, by type, help text and label names, and every gateway series `monitoring/` queries. The audio series and the gateway's `process_*` series, which the overview dashboard reads and which the gateway's registry has no collector for, are its known gaps (#426). It passed unchanged before the change.
- `app.py`, `audio_storage.py`, `auth.py` and `rate_limiter.py` are off the mypy ignore list.
...
After PR7b the adapter has no production consumer: no gateway module imports it, and `LEGACY_SESSION_IMPORTERS` in `tests/test_gateway_dependency_ownership.py` is empty. Its remaining consumers are tests: `test_session_manager.py`, `test_session_timeout.py`, `test_session_lifecycle_emission.py`, `test_content_retention.py`, `test_end_to_end_conversation.py` and `test_pipeline_metadata_integration.py` construct it, and `test_realtime_protocol_guard.py` allowlists its inline frames. It stays so it can be removed later in one step, with those tests.
...
- `LegacySessionManager` (`legacy_session_manager.py`) is the compatibility adapter: the `str`-keyed path and its own Redis persistence, moved unchanged. Only tests construct it; the unregistered `services/api_gateway/session.py`, its one production-tree consumer, went in PR7b. `test_only_the_legacy_adapter_reaches_the_legacy_session_manager` fails if any gateway module imports it.
...
- `routes/session.py` keeps `GET /languages/supported`; PR7b removed its unregistered leftovers. It re-exports nothing that moved.
...
- `call_ai_service` takes its breaker. Each app's `ServiceHealthManager` builds its breakers directly rather than from the process-wide `CircuitBreakerFactory`, so two apps share none; the status routes read them through the circuit breaker client. PR7b deleted the factory, which nothing in production used after this.
...
- `AUDIO_BASE_DIR`, `ORIGINAL_AUDIO_DIR` and `TRANSLATED_AUDIO_DIR` were still read at import, for the legacy functions with no production caller that task 4.2 inventoried. PR7b deleted them with those functions, so nothing reads `SSF_AUDIO_BASE_DIR` at import.
...
- `WebSocketMetrics` (`websocket_monitor.py`) holds every realtime Prometheus series, `websocket_monitor_initialized` included. It held `websocket_polling_messages_dropped_total` too, which nothing counted once the fallback was unwired; PR7b removed it. A series registers once per registry, so `app.py` builds it once on the module registry, as it does the other process-wide series, and `create_app()` puts it on `app.state.websocket_metrics`; PR7a moved it into each app's `GatewayMetrics` (above). Names, labels and help texts are unchanged; `tests/gateway_contract/test_contract_realtime_metrics.py` drives a real app and pins the families and label names `/metrics` exposes, and every series `monitoring/alert_rules.yml` and the Grafana dashboards query.
...
- `fallback_manager` is unwired: no import or registry binding in `app.py`, no container field, no `websocket_fallback_task`. PR7b deleted `websocket_fallback.py`.
...
- `TenantSessionManager.heartbeat_received` no longer calls `send_to_client`, which `WebSocketManager` never had. A heartbeat changes no tenant session state, as before; the legacy manager's method is unchanged. PR7b deleted the tenant method, a no-op only tests called.
...
- `websocket_fallback.py`, `get_websocket_stats` and `websocket_connection_test` are untouched; PR7b deleted them. #348 still decides the public monitoring scope, and this slice adds, removes or changes no endpoint.

#### Cleanup (PR7b)

- Deleted, after a fresh consumer search in code, tests, docs, scripts, compose files, Dockerfiles and CI workflows: `websocket_fallback.py`; `enhanced_audio_validation.py`; the unregistered `services/api_gateway/session.py`; the unregistered helpers and models in `routes/session.py`, which keeps its one route, `GET /api/languages/supported`; `get_websocket_stats`, `websocket_connection_test`, `WebSocketManager.get_connection_stats`, the reconnect attributes and `_calculate_reconnect_delay`; `AdaptivePollingManager.client_profiles`; `MessageType.DEVICE_ORIENTATION_CHANGE` and `RECONNECT_REQUIRED` and `ConnectionState.HEARTBEAT_TIMEOUT`; the legacy functions and import-time directories of `audio_storage.py`; `CircuitBreakerFactory` and the root `conftest.py` fixture that reset its registry; `TenantSessionManager.heartbeat_received`; and `websocket_polling_messages_dropped_total`. characterization.md lists each with its consumers.
- `app.routes` (55 routes, both WebSocket routes included) and the OpenAPI document are identical to `9f05640`. `/metrics` is identical except for the one family removed on purpose; `test_contract_metrics_surface.py` and `test_contract_realtime_metrics.py` drop it.
- The deleted modules, and nine more that are now clean under both mypy 1.13 and the pinned 2.3.1, leave the mypy ignore list: `quality_telemetry`, `quality_telemetry_otlp`, `routes.login`, `routes.session`, `studio_login_directory_client`, `studio_runtime_client`, `studio_runtime_flow`, `tenant_session` and `translation_refiner`.

### Decision: The tenant session store's async persistence port is its own change

Decided in PR4a, which assessed it at roughly 600–900 lines, and recorded here as task 2.2b. `RedisTenantSessionStore` calls Redis with the synchronous client from async request and WebSocket handlers, so every session read and write blocks the event loop for one round trip. The realtime ticket store shares the same connection, so both must move together, and the synchronous session access guards, which run in the thread pool today, would move onto the event loop. That is a change of its own with its own evidence, not a cleanup: #428 tracks it, with `redis.asyncio` for both stores or an interim `asyncio.to_thread` adapter as the options. This change keeps the synchronous store.
