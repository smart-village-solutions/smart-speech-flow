...
class EnhancedAudioValidator:
        ...
        for signature, format_name in self.BROWSER_FORMATS.items():
            if audio_bytes[:len(signature)] == signature or signature in audio_bytes[:100]:
                ...
                return AudioFormatDetection(
                    ...
                    mime_type=format_info.get('mime_types', ['application/octet-stream'])[0],
                    is_wav=False,
                    is_supported_by_browser=True,
                    needs_conversion=True,
    ...
    def validate_and_convert_audio(self, audio_bytes: bytes) -> Tuple[bool, bytes, str, dict]:
        ...
        logger.info(f"Audio-Format erkannt: {format_detection.format_name} (Konfidenz: {format_detection.confidence})")
        ...
        if format_detection.is_supported_by_browser and self.ffmpeg_available:
            try:
                converted_audio = self._convert_with_ffmpeg(audio_bytes, format_detection.format_name)
                if converted_audio:
                    details.update({
                    ...
                    try:
                        ...
                        logger.info(f"Erfolgreiche Konvertierung: {format_detection.format_name} -> WAV")
                        return True, converted_audio, "", details

                    except Exception as conv_wav_error:
                        details['conversion_wav_error'] = str(conv_wav_error)
                        logger.error(f"Konvertierte WAV-Datei ungültig: {conv_wav_error}")
    ...
    def _convert_with_ffmpeg(self, audio_bytes: bytes, source_format: str) -> Optional[bytes]:
        ...
        try:
            ...
            if result.returncode == 0 and os.path.exists(temp_output):
                ...
                logger.info(f"FFmpeg-Konvertierung erfolgreich: {len(audio_bytes)} -> {len(converted_audio)} bytes")
                return converted_audio
            else:
                logger.error(f"FFmpeg-Fehler (Code {result.returncode}): {result.stderr.decode()}")
                return None
...
def enhanced_validate_audio_input(audio_bytes: bytes, normalize: bool = True) -> 'AudioValidationResult':
    """
    Erweiterte Audio-Validierung mit Multi-Format-Support
    Drop-in Replacement für validate_audio_input
    """
    import time
    from services.api_gateway.pipeline_logic import AudioValidationResult, AudioSpecs
    ...
    success, converted_audio, error_message, format_details = validator.validate_and_convert_audio(audio_bytes)
    ...
    try:
        if 'channels' in format_details:
            ...
            duration_seconds = format_details.get('duration_seconds', frames / sample_rate if sample_rate > 0 else 0)
    ...
    if duration_seconds < specs.MIN_DURATION_SECONDS:
        spec_errors.append(f"Duration too short: {duration_seconds:.2f}s (min: {specs.MIN_DURATION_SECONDS}s)")
    if duration_seconds > specs.MAX_DURATION_SECONDS:
        spec_errors.append(f"Duration too long: {duration_seconds:.2f}s (max: {specs.MAX_DURATION_SECONDS}s)")
