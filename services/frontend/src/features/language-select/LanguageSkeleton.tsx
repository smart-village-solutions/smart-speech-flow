 ...
 */
export function LanguageSkeleton({ label }: Readonly<{ label: string }>) {
  return (
    // <output> carries role="status" itself, so the role attribute is redundant.
    <output aria-label={label} aria-busy="true" className="mx-auto flex max-w-sm flex-col gap-4">
      {Array.from({ length: PLACEHOLDER_ROWS }, (_, index) => (
        <div key={index} className="flex animate-pulse items-center gap-6 px-4 py-2">
