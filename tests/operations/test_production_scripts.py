...
if [[ "${{1:-}}" == "ps" ]]; then
  printf '%s\\n' '{services}'
elif [[ " $* " == *" exec -T prometheus "* ]]; then
  printf 'Prometheus Server is Healthy\\n'
elif [[ " $* " == *" exec -T redis "* ]]; then
  printf 'PONG\\n'
elif [[ " $* " == *" exec -T clickhouse "* ]]; then
  printf '1\\n'
elif [[ " $* " == *" exec -T keycloak "* ]]; then
